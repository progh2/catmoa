...
| 기능 | 설명 |
|---|---|
| 고양이 플로팅 위젯 | 항상 위에 떠 있는 작은 텍스트 고양이. 상황에 따라 표정이 바뀜 (처리 중이면 🍙 먹는 중). `assets/cat/`(또는 설정 폴더 `cat/`)에 PNG를 넣으면 **이미지 고양이**로 교체 — 규격은 [assets/cat/README.md](assets/cat/README.md) |
...
- **터미널로 격리 속성 제거**: `xattr -cr /Applications/catmoa.app` (내려받은 파일에 붙는 `com.apple.quarantine` 속성을 지웁니다)
