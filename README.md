...
| 정보 이름 | 누가 입력하는지 | 외부 전송 | 저장 위치·기간 | 볼 수 있는 사람 | 삭제 방법·시점 |
|---|---|---|---|---|---|
| 입력 자료 원문 (HWP·PDF·이미지·복사한 텍스트) | 교사가 직접 드래그·붙여넣기 | ❌ 원문 그대로는 보내지 않음 (마스킹 후 전송) | 저장하지 않음 — 메모리에서만 처리 | 교사 본인 | 앱을 닫으면 사라짐 (별도 삭제 불필요) |
| 쿨메신저 받은 쪽지 | 앱이 자동 감지 (기본 꺼짐, Windows 전용) | ❌ 원문 그대로는 보내지 않음 | 원본 DB는 읽기 전용 **임시 복사본**으로만 열고, 읽는 즉시 삭제 | 교사 본인 | 읽기 직후 자동 삭제 · 설정에서 연동 끄기 |
| AI에 보내는 본문 (마스킹된 텍스트) | 앱이 원문에서 자동 생성 | ⭕ **선택한 AI 공급자로 전송** (Ollama 선택 시 전송 없음) | 앱에는 저장하지 않음. 공급자 보관 정책은 각 사업자 약관에 따름 | 교사 본인 + 해당 AI 공급자 | 로컬 LLM(Ollama)으로 바꾸면 전송 자체가 없어짐 |
| 이미지·스크린샷 | 교사가 붙여넣기·드롭 | ⭕ 비전 모델로 전송 | 저장하지 않음 | 교사 본인 + 해당 AI 공급자 | 앱을 닫으면 사라짐 · **이미지 속 글자는 마스킹되지 않음**에 유의 |
| 추출된 일정·할 일 (제목·날짜·장소·메모) | AI가 제안 → **교사가 검토·수정·확정** | ⭕ 확정한 항목만 Google로 전송 | 교사 본인의 Google 캘린더·Tasks (구글 계정 정책에 따라 보관) | 교사 본인 (공유하지 않으면 남에게 안 보임) | 캘린더·Tasks에서 직접 삭제 |
| 일정에 함께 넣는 **원문 발췌** | 앱이 자동 첨부 (기본 1,500자, 마스킹 **안 함**) | ⭕ Google로 전송 | 캘린더 설명·할 일 메모 하단 | 교사 본인 | 설정 → 일반 → '원문 포함 길이'를 **0**으로 두면 넣지 않음 · 항목 삭제 시 함께 삭제 |
| Google 계정 (이메일·OAuth 토큰) | 교사가 브라우저에서 로그인 | ⭕ Google 인증 서버 | OS 키체인(Keychain/자격 증명 관리자/Secret Service), 실패 시 설정 폴더 `secrets.json` | 교사 본인 | 설정 → Google → **로그아웃** (토큰 즉시 삭제) |
| AI API 키 | 교사가 설정 화면에 입력 | ⭕ 해당 AI 공급자 (인증용) | 위와 같은 OS 키체인 | 교사 본인 | 설정에서 키를 지우고 저장 |
| 앱 설정 (내 역할·시간표·분류 규칙·쿨메신저 폴더·창 위치) | 교사가 설정 화면에 입력 | ❌ (단, '내 역할'·시간표는 판단 기준으로 **AI 프롬프트에 포함**) | 설정 폴더 `config.json` — 지울 때까지 | 교사 본인 | 설정 폴더를 지우면 초기화 |
| 개인정보 마스킹 모델 (선택) | 교사가 설정에서 내려받기 | ❌ (내려받은 뒤에는 인터넷 없이 동작) | 설정 폴더 `pii_model/` 약 295MB | 해당 없음 (개인정보 아님) | 설정 → 개인정보 → **모델 삭제** |
| 실행 기록(로그) | 앱이 자동 생성 | ❌ | **파일로 남기지 않음** (콘솔 출력만) | 교사 본인 | 앱 종료 시 사라짐 |
| 업데이트 확인 | 앱이 자동 (설정에서 끌 수 있음) | ⭕ GitHub — 보내는 값은 현재 버전 번호뿐 | 저장하지 않음 | 해당 없음 | 설정 → 업데이트 → 자동 확인 끄기 |

> **설정 폴더 위치** — Windows `%LOCALAPPDATA%\catmoa`, macOS `~/Library/Application Support/catmoa`, Linux `~/.config/catmoa`.
> 이 폴더를 통째로 지우면 설정·토큰·내려받은 모델이 모두 사라집니다.
> 서버와 텔레메트리가 없어서, 제작자를 포함한 **누구도 사용자의 자료를 볼 수 없습니다**.
