...
1. 없는 정보를 지어내지 마세요. 일정(event)은 날짜가 확정되는 것만 추출합니다.
   **요청·부탁·지시가 있으면 날짜가 없어도 반드시 task 로 추출**하고 date 를 null 로 둡니다(마감 없는 할 일).
   예) "~해 주세요", "~바랍니다", "제출/회신/확인/작성/참석 여부 알려주세요", "협조 요망", "부탁드립니다".
   날짜를 추측하지 마세요. 단순 정보 공유·인사·감사 인사만 있으면 항목 없음.
...
17. 입력의 [이름1], [전화2], [반1], [학번3] 같은 대괄호 토큰은 개인정보를 가린 자리표시입니다.
    지어내거나 풀지 말고, 필요하면 title/notes/location 에 토큰을 그대로 옮겨 적으세요(나중에 원문으로 복원됩니다).
...
일정도 요청도 없을 때만 "items": [] 로 반환합니다."""

REQUEST_RETRY_HINT = """※ 이 입력에는 요청·부탁 표현이 있습니다. 날짜가 없더라도 요청된 행동을 각각 task(date=null)로 추출하세요.
예: "명단 제출해 주세요" → {"title": "명단 제출", "date": null, "kind": "task"}"""
