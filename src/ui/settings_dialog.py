...
class SettingsDialog(QDialog):
    ...
    def _build_coolm(self) -> QWidget:
        ...
        self.coolm_history = QSpinBox()
        self.coolm_history.setRange(0, 5000)
        self.coolm_history.setSingleStep(100)
        self.coolm_history.setSuffix("자")
        self.coolm_history.setSpecialValueText("제외")
        self.coolm_history.setValue(s.history_chars)
        self.coolm_history.setToolTip("답장에 쌓인 이전 대화는 분리해 '참고용'으로만 붙입니다. 최근 내용이 우선 분석됩니다.")
        form.addRow("이전 대화 참고 길이", self.coolm_history)
        lay.addWidget(box)
    ...
    def _save(self):
        ...
        cm.history_chars = self.coolm_history.value()
