...
## 컨텍스트 앵커
- intent: v1.4.16 — PC 시계 오차 검출(#64): `src/netdiag.py` — HTTP Date 헤더(gstatic/captive.apple 후보)로 clock_skew_seconds, 5분 이상이면 경고. 구글 로그인 CERTIFICATE_VERIFY_FAILED 에 ssl_failure_hint(시계 크면 시계 안내, 아니면 학교망 보안 장비 안내) 덧붙임, `--selftest` 'PC 시계' 줄. HTTPS 는 시계 틀어지면 그 자체가 실패하므로 반드시 HTTP 로 확인
